...
module.exports = class extends Controller {
  ...
  async checkExists(email) {
    return new Promise((resolve, reject) => {
      this.db.find({ email: formatEmail(email) }, false).then((result) => {
          let check = [];
          result.data.map((value) => check.push(value.email));
          if (check.includes(email) == true) {
    ...
    try {
      ...
      if (_check == false && req.account) {
        ...
        if (
          ...
          _account.permissions = await this.getPermissions(_account.id,_account.role_id);
  ...
  async getAll(req, res) {
      try {
          ...
          this.response(res, 200, ( _data == null ? [] : _items));
      } catch (e) {
          this.response(res, 500, e.message);
      }
  }
  ...
  async checkRole(req,role_id=''){
    return new Promise(async(resolve, reject) => {
      ...
      if(role_id != ''){
          ...
          if (result) {
              let _module = req.route.path.split("/")[1]??'';
  ...
  async getPermissions(_id, _role_id){
    try {
